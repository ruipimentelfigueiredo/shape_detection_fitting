	...
	*/
	...
	if (use_ransac)
	{
		...
		boost::shared_ptr<CylinderFittingRansac> cylinder_fitting(new CylinderFittingRansac((float)normal_distance_weight,(unsigned int)max_iterations,(unsigned int)distance_threshold,(float)min_radius, (float)max_radius));
	        ...
	        boost::shared_ptr<ShapeDetectionManager<CylinderFittingRansac> > shape_detection_manager(new ShapeDetectionManager<CylinderFittingRansac>(cylinder_classifier,cylinder_fitting,cam_intrinsic,classification_threshold));
	...
	else
	{
		...
		boost::shared_ptr<CylinderFittingHough> cylinder_fitting(new CylinderFittingHough(gaussian_sphere,(unsigned int)angle_bins,(unsigned int)radius_bins,(unsigned int)position_bins,(float)min_radius, (float)max_radius,(float)accumulator_peak_threshold,(unsigned int)mode));
	        ...
	        boost::shared_ptr<ShapeDetectionManager<CylinderFittingHough> > shape_detection_manager(new ShapeDetectionManager<CylinderFittingHough>(cylinder_classifier,cylinder_fitting,cam_intrinsic,classification_threshold));

		CylinderFittingROS<CylinderFittingHough> cylinder_fitting_ros(n, n_priv,shape_detection_manager);
