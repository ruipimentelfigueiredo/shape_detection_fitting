    	...
    	void ShapeDetectionFittingRosNodelet::onInit()
	{
		...
		if (use_ransac)
		{
			...
			boost::shared_ptr<CylinderFittingRansac> cylinder_fitting(new CylinderFittingRansac((float)normal_distance_weight,(unsigned int)max_iterations,(unsigned int)distance_threshold,(float)min_radius, (float)max_radius));
		        ...
		        boost::shared_ptr<ShapeDetectionManager<CylinderFittingRansac,SphereFittingHough> > shape_detection_manager(new ShapeDetectionManager<CylinderFittingRansac,SphereFittingHough>(cylinder_classifier,cylinder_fitting,sphere_fitting,cam_intrinsic,classification_threshold));

			inst_ransac_fitting_.reset(new ShapeFittingROS<CylinderFittingRansac,SphereFittingHough>(getNodeHandle(),getPrivateNodeHandle(),shape_detection_manager));
		}
		else
		{
