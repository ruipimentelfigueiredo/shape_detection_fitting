...
int main (int argc, char** argv)
{
	...
	if (argc < 3)
	{
		std::cout << "invalid number of arguments: rosbag_dir iterations heights"<< std::endl;
		exit(-1);
	}

    	std::string rosbag_dir   = std::string(argv[1]);
	unsigned int iterations_ = atoi(argv[2]);
	...
	*/
	...
	unsigned int iterations=height*iterations_;
	while(ros::ok())
	{
