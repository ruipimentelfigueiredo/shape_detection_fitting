...
int main (int argc, char** argv)
{
	...
	static std::string heights_ = std::string(argv[8]);  // segmentation threshold for mask
	std::cout << "threshs: " << heights_<< std::endl;
		...
		for(unsigned int i=0; i<iterations; ++i)
		{
			for(unsigned int h_=0; h_<heights.size();++h_)
			{
				for(unsigned int r_=0; r_<radii.size();++r_)
				{
					...
					ROS_INFO_STREAM(" iteration:" << i << " noise_level: "<< noise_levels[n] << " height: " << heights[h_] << " radius: " << radii[r_] << " total iteration: "<< r_+h_*radii.size()+i*radii.size()*heights.size()+n*radii.size()*heights.size()*iterations); 
				}
			}
		}
	}
